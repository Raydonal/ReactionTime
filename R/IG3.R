# amended 27_11_2007
IG3 <-function (mu.link = "log", sigma.link = "log") 
{   
    ...
    structure(
          list(family = c("IG3", "Inverse Gaussian"),
        ...
        sigma.linkinv = dstats$linkinv,
          ...
          G.dev.incr  = function(y,mu,sigma,...) 
                               {  -2*dIG3(y,mu,sigma,log=TRUE)},
                rqres = expression(rqres(pfun="pIG3", type="Continuous", y=y, mu=mu, sigma=sigma)),
...
#----------------------------------------------------------------------------------------  
dIG3<-function(x, mu = 1, sigma = 1, log=FALSE)
...
#----------------------------------------------------------------------------------------  
pIG3 <- function(q, mu = 1, sigma = 1, lower.tail = TRUE, log.p = FALSE)
  {    #  browser() 
    if (any(mu < 0))  stop(paste("mu must be positive", "\n", "")) 
    if (any(sigma < 0))  stop(paste("sigma must be positive", "\n", "")) 
...
#----------------------------------------------------------------------------------------  
qIG3 <- function(p, mu=1, sigma=1,  lower.tail = TRUE, log.p = FALSE)
 {
    #---functions--------------------------------------------   
       h1 <- function(q)
       { 
     pIG3(q , mu = mu[i], sigma = sigma[i])-p[i]   
       }
       h <- function(q)
       { 
     pIG3(q , mu = mu[i], sigma = sigma[i])   
       }
     #-------------------------------------------------------
    if (any(mu <= 0))  stop(paste("mu must be positive", "\n", "")) 
    if (any(sigma <= 0))  stop(paste("sigma must be positive", "\n", ""))      
